...
export default function LeadForm() {
  ...
  const submit = async () => {
    ...
    setDone(null);
    try {
      const res = await fetch("/api/ecodrive/pasajero-invite", {
        method: "POST",
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="flex gap-2">
          <span className="bg-zinc-100 border rounded-lg px-3 py-3 text-zinc-600 font-medium">+51</span>
          <input
            type="tel"
            value={phone}
        ...
        <p className="text-xs text-zinc-500 mt-1">
          Solo el número, sin código país.
        </p>
      </div>
      ...
      {done && (
        <div className={`p-3 rounded-lg text-sm ${done.ok ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}>
          {done.msg}
        </div>
      )}
    </div>
